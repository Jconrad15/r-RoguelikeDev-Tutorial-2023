...
public class Path_AStar
{
    ...
    public Path_AStar(
        Tile tileStart, Tile tileEnd,
        GameMap gameMap, EntityManager entityManager)
    {
        ...
        Path_TileGraph tileGraph = gameMap.TileGraph;

        // Check to see if there is a valid tile graph
        if (tileGraph == null)
        {
            gameMap.UpdateTileGraph();
        ...
        Path_Node<Tile> a, Path_Node<Tile> b)
    {
        ...
        return Mathf.Sqrt(
               Mathf.Pow(a.data.position.Item1 - b.data.position.Item1, 2) + 
               Mathf.Pow(a.data.position.Item2 - b.data.position.Item2, 2));
    }
    ...
    private float Dist_Between(Path_Node<Tile> a, Path_Node<Tile> b)
    {
        ...
        if (Mathf.Abs(a.data.position.Item1 - b.data.position.Item1) +
            Mathf.Abs(a.data.position.Item2 - b.data.position.Item2) == 1)
        {
            return 1f;
        }
        // Diagonal neighbours have a distance of 1.41421356237
        if((Mathf.Abs(a.data.position.Item1 - b.data.position.Item1) == 1 &&
            Mathf.Abs(a.data.position.Item2 - b.data.position.Item2) == 1))
        {
